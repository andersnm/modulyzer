...
export class Player {
    ...
    play() {
        ...
        this.scheduleSequence(this.startTime, this.currentTime, 1);
        this.currentTime = 1;

        this.playInterval = setInterval(() => {

            const until = this.context.currentTime + 1;
    ...
    schedulePattern(pattern: Pattern, deviceTime: number, fromTime: number, duration: number) {
        ...
        for (let column of pattern.columns) {
            ...
            for (let event of column.events) {
                if (event.time >= fromTime && event.time < fromTime + duration) {
                    if (pin.type === "controller") {
                        console.log("Sending controller", column, deviceTime, event.time, 0xB0 + pin.value, event.value, 0);
    ...
    scheduleSequence(deviceTime: number, fromTime: number, duration: number) {
        ...
        for (let column of this.sequence.columns) {
            for (let ev of column.events) {
                ...
                if (areOverlapping(ev.time, ev.time + p.duration, fromTime, duration)) {
                    ...
                    this.schedulePattern(p, deviceTime + ev.time, fromTime - ev.time, duration);
                }
            }
        }
    }
